...
no hand-built 3D scene to maintain and no downloaded asset the build depends on:
a world is a description file plus its inputs - road and building geometry from
OpenStreetMap, elevation from an open DEM, and textures authored here - and a
generator turns them into a scene. Committing the inputs is what makes a build
